...
| 0.3.0 (candidate) | `INSTRUMENTED_STANDALONE`, `PREBUILT_STANDALONE` | Receipt-backed standalone behavior passed, or an opaque EXE met the narrower launch-only contract |

Current sealed release: `0.2.0`. Candidate `0.3.0-rc.1` implements all four modes. P1/P2 remain sealed to `StandaloneWindows64 + Mono`. P3 Mono passed the complete three-version acceptance matrix; P3 IL2CPP is implemented but remains fail-closed because none of the three installed Unity editors currently has Windows x64 IL2CPP Build Support. No IL2CPP tuple is registered, and the `v0.3.0` release tag is intentionally withheld until that matrix passes.
...
See [architecture.md](docs/architecture.md) for trust boundaries, [the Skill guide](docs/skills/unity-player-verification.md) for operator usage, and [the P3 acceptance record](docs/validation/unity-player-verification-p3-real-unity-acceptance.md) for the current Mono approval and IL2CPP release blocker.
