...
Backend availability is derived from the selected editor's Windows Support `Variations` tree. A shared editor `Data/il2cpp` directory is not sufficient evidence of Windows IL2CPP Build Support. Candidate `0.3.0-rc.1` has approved Mono tuples only; IL2CPP execution remains blocked until the exact Windows x64 non-development variation and native toolchain pass the full real-Unity matrix.
